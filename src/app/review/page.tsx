...
const SUMMARY: { label: string; status: ReviewStatus; tone: string }[] = [
  { label: "Approved", status: "approved", tone: "text-teal" },
  { label: "Pending", status: "pending", tone: "text-accent" },
  { label: "Needs changes", status: "needs-changes", tone: "text-rose" },
];
    ...
    <main className="mx-auto w-full max-w-6xl px-6 py-12">
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        {SUMMARY.map((entry) => (
          ...
          >
            <p className={`text-3xl font-semibold ${entry.tone}`}>
              {countByStatus(entry.status)}
            </p>
            <p className="mt-1 text-sm text-muted">{entry.label}</p>
          </div>
        ))}
      </div>
      ...
      <section className="mt-12 rounded-2xl border border-border bg-surface/40 p-6">
        ...
        <p className="mt-2 text-sm leading-6 text-muted">
          ...
          <span className="text-foreground">reviewed by a human</span>, and only
          fiction-only, private-material-free items pass. The shape of that
          boundary lives in the docs — see the adoption adapter and boundary map
          in{" "}
